    ...
    return (
      <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center p-4">
        <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-8 rounded-2xl backdrop-blur-md max-w-md text-center shadow-2xl">
          ...
          <p className="text-sm opacity-80 mb-6">Ce lien d'invitation n'est plus valide ou la campagne n'existe pas.</p>
          <button
            onClick={() => navigate('/campaigns/explore')}
            className="px-6 py-2 bg-red-500/20 hover:bg-red-500/30 rounded-xl font-bold text-red-300 transition-colors"
          >
    ...
    <div className="min-h-screen bg-[#0a0a0f] font-sans relative overflow-hidden">
      ...
      <nav className="sticky top-0 z-50 border-b border-white/10 bg-[#0a0a0f]/90 backdrop-blur-xl">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <Link to="/campaigns/explore" className="text-xs text-violet-400 hover:text-violet-300 font-semibold">
            Explorer
          </Link>
        </div>
      </nav>
      ...
      {/* Content */}
      <div className="flex items-center justify-center p-4 min-h-[calc(100vh-56px)] relative z-10">
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl max-w-lg w-full shadow-2xl text-center">
          ...
          <div className="inline-block px-4 py-1.5 bg-orange-500/20 border border-orange-500/30 text-orange-400 rounded-full text-xs font-bold mb-6">
            🔒 {campaign.visibility === 'UNLISTED' ? 'Campagne Non Répertoriée' : 'Campagne Privée'}
          </div>
            ...
            <div className="flex justify-between items-end mb-2">
              <div>
                ...
                <span className="text-xs text-gray-500 block">
                  sur {target.toLocaleString('fr-TN')} TND
                </span>
              </div>
          ...
          {user ? (
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 p-5 rounded-xl">
              <p className="text-blue-300 text-sm mb-4 font-medium">
                Connectez-vous pour participer à cette campagne.
              </p>
              <Link
                to={`/login?redirect=/campaigns/join/${code}`}
                className="block w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors shadow-lg text-center"
              >
